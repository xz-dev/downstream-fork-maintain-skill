...
2. **Compatibility branch** — when the patch's real parent in the sequence is itself a squash-integrated commit on the rebuilt release branch. Squash integration records no ancestry, so merge-base falls back to upstream and every downstream-touched file reports add/add conflicts. Build a compat branch with at least two commits: a *base* commit whose tree is the accumulated release candidate the patch expects to land on, and a *tip* commit with the patch's resolved content. The sync step then applies `git diff <base> <tip> | git apply --3way`, which is ancestry-irrelevant — only trees matter. Use an explicit `base..tip` range (e.g. `origin/patch/foo-accumulated-base` and `origin/patch/foo-accumulated-tip`), not `tip^..tip`: later fixups on the tip must not shrink the applied delta.

   The base commit's tree is the accumulated result of *every* patch earlier in the integration order, so construction cost grows with position. That is a real cost for late-sequence patches, but compat is not wrong for them: the alternative is a resolver that re-implements merge semantics in strings. A compat branch also does not need rebuilding on every upstream advance — only when the accumulated tree it was built against actually conflicts. Semantic patch changes (an upstream API the patch calls was renamed or removed) belong on the patch branch, not inside the compat tip; the compat branch then applies the already-adapted patch.

   Hand-resolved unions must be compile-gated (`tsgo`/`biome` or the repo's checker) before pushing — diff-shape comparison against the release branch cannot catch a missing or extra brace in a different pre-image.

3. **Narrow resolver script on `ci`** — acceptable for mechanical-only conflict shapes (import reordering, adjacent field insertion) that produce no new runtime semantics. A resolver that rewrites function signatures, splits control flow, or drops/merges whole blocks is not narrow — that logic belongs on the patch or compat branch where it can be tested. Line count is not the test; semantic weight is.
